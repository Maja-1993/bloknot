...
def edit_data():
    remove_empty_lines("telef1.csv")
    remove_empty_lines("telef2.csv")
    ...
    if choice == 1:
        ...
        with open("telef1.csv", 'w', encoding='utf-8') as f:
            f.writelines(data)
        ...
        new_data = name_data() + "; " + surname_data() + "; " + phone_data() + "; " + adres_data()

        data[line_number-1] = new_data + "\n"

        with open("telef2.csv", 'w', encoding='utf-8') as f:
            f.writelines(data)
        print ("Изменения сохранены")
